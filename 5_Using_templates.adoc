...
Click on the *Add to Project -> Browse Catolog* button and you will be taken to the application creation screen shown
below, where you can select an image or template.

image:images/template-add-to-project.png[image]

Scroll down and click on the *Data Stores* session. Find the *MySQL (Ephemeral)*
template from the list and click *Select*. Also notice the *MySQL
(Persistent)* template...we will address persistent templates a little later in a different lab
exercise.
